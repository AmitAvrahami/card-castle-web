...
import CardQuantityModal from "../../CardQuantityModal/CardQuantityModal";
...
function CreateDeck() {
    ...
    const handleSaveCard = ({ cardId, quantity, imageUrl }) => {
        setPickedCards((prev) => {
            ...
            const totalQuantity = existingCard ? existingCard.quantity + quantity : quantity;
    ...
    return (
        <div className="create-deck-container">
            ...
            {/* Primary Modal */}
            <SearchModal show={findCardShow} onHide={handleCloseFindCardShow} centered onCardClick={handleCardClick} />
            ...
            {/* Publish Deck Modal */}
            <Modal show={publishDeckShow} onHide={handleClosePublishDeckShow} centered>
            ...
            <Modal show={deleteCardShow} onHide={handleCloseDeleteCardShow} centered className="small-modal">
                <Modal.Header closeButton>
            ...
            <div className="picked-cards-grid">
                {pickedCards.length > 0 ? (
                    pickedCards.map((card) => (
                        Array(card.quantity).fill(null).map((_, idx) => (
                            <Col key={`${card.cardId}-${idx}`} xs={12} sm={6} md={4} lg={2} className="d-flex justify-content-center mb-4">
                                <Card className="card-custom" onClick={() => handleDeleteCard(card)}>
                                    <Card.Img
                                        variant="top"
                                        src={card.imageUrl || 'https://via.placeholder.com/150x200'}
